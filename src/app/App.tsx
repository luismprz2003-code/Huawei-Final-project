...
export default function App() {
  ...
  const [activeSection, setActiveSection] = useState<string>("cobertura-territorial");
  ...
  return (
    <div className="app-shell">
      ...
      <main className="dashboard-content" style={{ marginTop: '20px' }}>
        {/* NUEVO MENÚ DE NAVEGACIÓN SUPERIOR (TARJETAS COMO PESTAÑAS) */}
        <div style={{ 
          display: 'flex', 
          gap: '16px', 
          overflowX: 'auto', 
          ...
          {[
            { 
              id: "cobertura-territorial", title: "1. Cobertura", icon: "📡", statLabel: "Líder 5G",
              statValue: getTopRecordByMetric(filteredRecords, "poblacion_en_localidades_con_5g_garantizada_pct")?.state || "N/D", color: "#e11d48" // Tono Huawei (Rojo)
            },
            { 
              id: "cobertura-poblacional", title: "2. Población", icon: "👥", statLabel: "Prom. Hogares",
              statValue: `${getMetricAverage(filteredRecords, "hogares_en_localidades_con_internet_pct").toFixed(1)}%`, color: "#475569" // Tono Huawei (Gris oscuro)
            },
            { 
              id: "brecha-territorial", title: "3. Brechas", icon: "⚖️", statLabel: "Brecha Media",
              statValue: `${getMetricAverage(gapRecords, "brecha_cobertura_movil_pp").toFixed(1)} pp`, color: "#0f172a" // Tono Huawei (Negro/Azul muy oscuro)
            },
            { 
              id: "adopcion-cobertura", title: "4. Adopción", icon: "📱", statLabel: "Líder Smartphone",
              statValue: getTopRecordByMetric(filteredRecords, "personas_con_smartphone_pct")?.state || "N/D", color: "#dc2626" // Tono Huawei (Rojo brillante)
            },
            { 
              id: "oportunidad", title: "5. Oportunidad", icon: "🎯", statLabel: "Top Oportunidad",
              statValue: [...opportunityRecords].sort((a,b) => b.opportunityScore - a.opportunityScore)[0]?.state || "N/D", color: "#b91c1c" // Tono Huawei (Rojo intenso)
            }
          ].map((section) => {
            ...
            return (
              <div 
                ...
                style={{ 
                  ...
                  padding: '16px', 
                  borderRadius: '12px', 
                  border: isActive ? `2px solid ${section.color}` : '1px solid #e2e8f0', 
                  cursor: 'pointer', 
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <span style={{ fontSize: '1.5rem', filter: isActive ? 'none' : 'grayscale(100%)' }}>{section.icon}</span>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '0.65rem', fontWeight: 'bold', color: '#94a3b8', textTransform: 'uppercase' }}>
                      {section.statLabel}
                    </div>
                    <div style={{ fontSize: '1rem', fontWeight: '800', color: isActive ? section.color : '#64748b' }}>
                      {section.statValue}
                    </div>
                  </div>
                </div>
                <h3 style={{ margin: '0', fontSize: '1rem', color: isActive ? '#0f172a' : '#64748b', fontWeight: '700' }}>
                  {section.title}
                </h3>
              </div>
            );
        ...
        {/* CONTENIDO DE LA SECCIÓN ACTIVA (TOMA TODO EL ESPACIO) */}
